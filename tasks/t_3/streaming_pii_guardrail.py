...
class PresidioStreamingPIIGuardrail:
    ...
    def __init__(self, buffer_size: int =100, safety_margin: int = 20):
        configs = {"nlp_engine_name": "spacy","models": [{"lang_code": "en", "model_name": "en_core_web_sm"}]}
        nlp_engine_provider = NlpEngineProvider(nlp_configuration=configs)
    ...
    def process_chunk(self, chunk: str) -> str:
        ...
        if len(self.buffer) > self.buffer_size:
            ...
            results = self.analyzer_engine.analyze(text_to_process, language="en")
...
def main():
    ...
    pr_guardrail: PresidioStreamingPIIGuardrail = PresidioStreamingPIIGuardrail()
    ...
    while True:
        ...
        for chunk in llm_client.stream(messages):
            if chunk.content:
                safe_chunk = pr_guardrail.process_chunk(chunk.content)
                if safe_chunk:
                    print(safe_chunk, end="", flush=True)
                    full_response += safe_chunk

        final_chunk = pr_guardrail.finalize()
        if final_chunk:
